...
outlier2 <- PCAdata[PCAdata$PC1>(-25)&PCAdata$PC1<0&
                      PCAdata$PC2>30&PCAdata$PC2<45&
                      PCAdata$group=='Solid Tissue Normal', 5]
...
row.names(TcgaGtex_expectedCount_filtered_gene_agr_outlierRemoved_matrix) <-
  TcgaGtex_expectedCount_filtered_gene_agr_outlierRemoved[,1]
...
TPN_cochange_logFC_Pval <-
  TPN_cochange_logFC_Pval[order(TPN_cochange_logFC_Pval$logFC.mean, decreasing=T),]
...
volcanoPlot('TPN_DEG_volcano.pdf', a, 0.01, 2)
...
gene_info <- data.frame(Symbol=gene_info$Symbol,
                        description=gene_info$description,
                        Full_name_from_nomenclature_authority=gene_info$Full_name_from_nomenclature_authority)
TPN_cochange_logFC_Pval_filter_order_anno <- merge(TPN_cochange_logFC_Pval_filter_order,
                                                   gene_info, by.x='GeneSymbol',
                                                   by.y='Symbol', all.x=T, sort=F)
write.csv(TPN_cochange_logFC_Pval_filter_order_anno, 
          file='TPN_cochange_logFC_Pval_filter_order_anno.csv')
...
labeledHeatmap(Matrix = modTraitCor, xLabels = colnames(dataTrait), 
